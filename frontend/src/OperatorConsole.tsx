...
type Tab =
  "map" | "conflicts" | "constraints" | "analytics" | "edit" | "coregister";
...
function OperatorConsole() {
  ...
  const [constraints, setConstraints] = useState<ConstraintsResponse | null>(
    null,
  );
    ...
    return (
      <div className="app">
        <header>
          <h1>GeoCadastra</h1>
          <p className="muted">
            Parcel-graph reconstruction — operator console
          </p>
        </header>
    ...
    <div className="app">
      ...
      {tab === "constraints" && (
        <ConstraintsPanel
          wardJobId={wardJobId}
          refreshToken={refreshToken}
          onLoaded={setConstraints}
        />
      )}
      {tab === "analytics" && (
        <AnalyticsPanel wardJobId={wardJobId} refreshToken={refreshToken} />
      )}
      {tab === "edit" && (
        <EditPanel wardJobId={wardJobId} prefill={prefill} onApplied={bump} />
      )}
      {tab === "coregister" && (
        <CoRegisterPanel wardJobId={wardJobId} onApplied={bump} />
      )}
    </div>
  );
}
